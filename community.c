...
//Setup Player
static void setupPlayerPeerCommunity(int id, TPeer *peer, xmlDocPtr doc, TSymTable *xlog){
	char xpath[1000]={[0]='\0'};
	char *xdynamic=NULL, *xsize;
	char pars[1000]={[0]='\0'}, *parameter=NULL, *value=NULL;
	char entry[1000]={[0]='\0'};
...
/*	xmlChar *xValue;
	int levels,i;*/
		...
		parameter = strtok_r(pars, separator, &last);
		while(parameter){
			sprintf(xpath,"/community/tier[%d]/peer/player/policy/parameter[@name=\"%s\"]",id+1,parameter);
			value = xgetOneParameter(doc,xpath);
			sprintf(entry+strlen(entry),"%s;",value); free(value);
			parameter = strtok_r(NULL, separator, &last);
		}
		...
		TPlayer *player= createPlayer(atoi(xsize),policy);
	...
	peer->setPlayer(peer,player);//Set HCache for peer
}
